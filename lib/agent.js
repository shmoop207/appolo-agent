...
class Agent extends appolo_event_dispatcher_1.EventDispatcher {
    ...
    addHookOnSend(hook) {
        return this._addHook(types_1.Hooks.OnSend, hook);
    }
    addHookOnError(hook) {
        return this._addHook(types_1.Hooks.OnError, hook);
    }
    addHookOnResponse(hook) {
        return this._addHook(types_1.Hooks.OnResponse, hook);
    }
    addHookOnRequest(hook) {
        return this._addHook(types_1.Hooks.OnRequest, hook);
    }
    addHookPreHandler(hook) {
        return this._addHook(types_1.Hooks.PreHandler, hook);
    }
    addPreMiddlewareHook(hook) {
        return this._addHook(types_1.Hooks.PreMiddleware, hook);
    }
    _addHook(name, hook) {
        if (!this._hooks[name]) {
            this._hooks[name] = [];
